...
func runSchemaChangeRandomLoad(
	...
	if t.ExportOpenmetrics() {
		runCmd = append(runCmd, "--histogram-export-format='openmetrics'")
		runCmd = append(runCmd, "--histograms="+t.PerfArtifactsDir()+"/stats.txt")
		runCmd = append(runCmd, fmt.Sprintf("--openmetrics-labels='%s'", clusterstats.GetDefaultOpenmetricsLabelString(t, c)))
	} else {
		// Save the histograms so that they can be reported to https://roachperf.crdb.dev/.
		runCmd = append(runCmd, " --histograms="+t.PerfArtifactsDir()+"/stats.json")
	}
